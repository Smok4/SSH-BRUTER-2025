...
func main() {
	...
	fmt.Print("Use combo.txt directly ? (y/n): ")
	useCombo, _ := reader.ReadString('\n')
	useCombo = strings.TrimSpace(useCombo)

	if strings.ToLower(useCombo) != "y" {
	...
	for scanner.Scan() {
		...
		if line != "" {
			...
			if len(parts) >= 2 && parts[0] != "" && parts[1] != "" {
				...
				items = append(items, []string{parts[0]})
			}
		}
	}
	return items
...
func createComboFile(reader *bufio.Reader) {
	...
	// Read usernames and passwords as simple lists
	usernames := []string{}
	passwords := []string{}

	uf, err := os.Open(usernameFile)
	if err != nil {
		log.Fatalf("Failed to open username file: %s", err)
	}
	defer uf.Close()
	...
	pf, err := os.Open(passwordFile)
	if err != nil {
		log.Fatalf("Failed to open password file: %s", err)
	}
	defer pf.Close()
